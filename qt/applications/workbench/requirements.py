...
def check_qt():
    ...
    try:
        import mantidqt.qtpy as qtpy
        actual_ver = qtpy.PYQT_VERSION
        if LooseVersion(actual_ver) < LooseVersion(required_ver):
            show_warning("Please check installation requirements:\n"
                         "{} {}+ is required (found v{}).".format(package_name, required_ver, actual_ver))
    except ImportError as exc:
        show_warning("Failed to import mantidqt.qtpy: '{}'\n".format(str(exc)) +
